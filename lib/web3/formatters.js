 ...
 *
 * @method inputTransactionFormatter2
 * @param {Object} transaction options
...
var inputTransactionFormatter2 = function (options){
    ...
    if (options.blockNumber) {
        tx.setValidUntilBlock(options.blockNumber + 88);
    }

    tx.setData(new Uint8Array(Buffer.from(options.data.slice(2), "hex")));
    if (options.bytecode) {
        tx.setData(new Uint8Array(Buffer.from(options.bytecode, "hex")));
    } 
    ...
    var sign = secp256k1.sign(new Buffer(hash.toString(), 'hex'), new Buffer(options.privkey, 'hex'))
